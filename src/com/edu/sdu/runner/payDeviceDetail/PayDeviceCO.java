...
public class PayDeviceCO {
	...
	public static void main(String[] args) {
		...
		try {
			...
			Path op2 = new Path("/usr/local/hadoop/file/PayDeviceCO");
			if (fs2.exists(op2)) {
				fs2.delete(op2, true);
			...
			String fileName = Sysmbol.startDay.substring(0, Sysmbol.startDay.length() - 4) + 
					"_pd_co.txt";
			String filePath = "/usr/local/hadoop/file/PayDeviceCO/" + fileName;
			WriteJson.doWriteFile("/usr/local/hadoop/file/PayDeviceCO/part-r-00000", filePath);
