...
public class NewDeviceOS {
	...
	public static void main(String[] args) {
		...
		try {
			...
			Path op2 = new Path("/usr/local/hadoop/file/NewDeviceOS");
			if (fs2.exists(op2)) {
				fs2.delete(op2, true);
			...
			String fileName = Sysmbol.startDay.substring(0, Sysmbol.startDay.length() - 4) + 
					"_nd_os.txt";
			String filePath = "/usr/local/hadoop/file/NewDeviceOS/" + fileName;
			WriteJson.doWriteFile("/usr/local/hadoop/file/NewDeviceOS/part-r-00000", filePath);
