...
public class NewDeviceCI {
	...
	public static void main(String[] args) {
		...
		try {
			...
			Path op2 = new Path("/usr/local/hadoop/file/NewDeviceCI");
			if (fs2.exists(op2)) {
				fs2.delete(op2, true);
			...
			String fileName = Sysmbol.startDay.substring(0, Sysmbol.startDay.length() - 4) + 
					"_nd_ci.txt";
			String filePath = "/usr/local/hadoop/file/NewDeviceCI/" + fileName;
			WriteJson.doWriteFile("/usr/local/hadoop/file/NewDeviceCI/part-r-00000", filePath);
