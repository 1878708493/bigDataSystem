...
public class NewUserPT {
	...
	public static void main(String[] args) {
		...
		try {
			...
			Path op2 = new Path("/usr/local/hadoop/file/NewUserPT");
			if (fs2.exists(op2)) {
				fs2.delete(op2, true);
			...
			String fileName = Sysmbol.startDay.substring(0, Sysmbol.startDay.length() - 4) + 
					"_nu_pt.txt";
			String filePath = "/usr/local/hadoop/file/NewUserPT/" + fileName;
			WriteJson.doWriteFile("/usr/local/hadoop/file/NewUserPT/part-r-00000", filePath);
